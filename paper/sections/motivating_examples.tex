...
% Section 8 -- motivating examples (framing only at v0.1).  Boltzmann-
% shaped collision operators on the lattice and the lattice-Boltzmann
...
will eventually be used for.  Full proofs of the H-theorem on the
lattice and of the fluid-limit convergence are deferred to subsequent
papers.}

\subsection{Boltzmann-Shaped Operators on the Lattice}
\label{subsec:boltzmann}

\placeholder{Define a Boltzmann-shaped collision operator on
$\Lambda_n$ (or its rescaling $\epsilon \Lambda_n$): a binary /
...
$H$-functional.  State (without proving) the expected
$H$-theorem and the expected equilibrium measures.  Cross-reference
to a future paper for the full proof.}

\paragraph{Motivating-example status.}
\todomark{motivating-example: the H-theorem on $\Lambda_n$ is
...
\placeholder{State the lattice-Boltzmann -> NS / Euler limit
...
\paragraph{Motivating-example status.}
\todomark{motivating-example: the lattice-Boltzmann -> NS / Euler
convergence is asserted here but not proved.  Full proof
scheduled for a subsequent paper.  See Table~\ref{tab:audit},
``Fluid (NS / Euler) limit ansatz'' row, which is by design
\texttt{STUB}.}
...
theory and the rigorous Boltzmann -> NS / Euler limit as the
...
of the H-theorem on $\Lambda_n$ and of the fluid-limit convergence
are scheduled for subsequent papers in this repository or in
