% function_spaces.tex
% Section 6 -- function spaces on the diamond progression.
% Discrete L^p / ell^p, Sobolev-analogue spaces ell^{k,p}, d-dependent
% scaling, embedding theorems.

\label{sec:function_spaces}

\placeholder{Section body -- write during Phase 3.  Define the
discrete $\ell^p$ and Sobolev-analogue $\ell^{k,p}$ spaces on
$\Lambda_d$; state and prove $d$-dependent scaling, density, and
embedding theorems.}

\subsection{Discrete $\ell^p$ Spaces}
\label{subsec:lp_spaces}

\placeholder{Define $\ell^p(\Lambda_d)$ for $1 \leq p \leq \infty$
using the counting measure on $\Lambda_d$.  Standard Banach-space
properties: completeness, separability for $p < \infty$, duality.}

\subsection{Sobolev-Analogue Spaces}
\label{subsec:sobolev_analogue}

\placeholder{Define $\ell^{k,p}(\Lambda_d)$ as the space of lattice
functions whose discrete-derivatives up to order $k$ (via the
discrete exterior derivative $\mathrm{d}$ of
...
\placeholder{$d$-dependent scaling of the norms under
...
$d$-dependent exponent relations.  Unit-cell volume from
Theorem~\ref{thm:unit_cell_volume} enters the rescaling factors.}
