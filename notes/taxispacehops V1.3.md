...
## 9\. Lorentz anisotropy: the hard falsification point
...
### 9.4 How bad, and the three ways out

Terrestrial Lorentz-symmetry tests with rotating cryogenic resonators bound c-anisotropy at the **10⁻¹⁸** level. A √3 effect is not off by a factor; it is off by roughly eighteen orders of magnitude. `[standard]`

This belongs on the most-dangerous list alongside the CMB acoustic peaks — arguably ahead of it, since it is a clean laboratory bound rather than a cosmological fit. It is also the known killer for **regular** lattices specifically, and it is why causal set theory abandoned lattice regularity: a Poisson sprinkling in Minkowski space is boost-invariant, and a regular lattice provably is not.

Three ways out; only the third keeps the lattice:

1. **Give up regularity** — random or aperiodic sprinkling. Costs the entire combinatorial structure derived above.  
2. **Make propagation diffusive at every scale** — then the cone is never observable, but there are no massless particles at all.  
3. **Exhibit a suppression mechanism** — anisotropy falling as a high power of (spacing / observation scale). This requires an actual mechanism, not an expectation, and it is the only option compatible with keeping the construction.

Until (3) exists, the framework predicts a measured c-anisotropy eighteen orders of magnitude above the bound. State this openly in the paper; a referee will find it in the first five minutes regardless, and finding it stated is very different from finding it omitted.

## 10\. Open items, ranked

0. **Light-cone anisotropy** (§9). Find a suppression mechanism, or accept that the framework is falsified at 10⁻¹⁸ by existing laboratory data. This outranks everything below it: a failure here is fatal, whereas failures below are merely unsupported claims. Do not put the construction in print without addressing it.  
